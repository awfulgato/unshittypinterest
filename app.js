(() => {
  ...
  async function load() {
    try {
      const response = await fetch(`/api/boards/${encodeURIComponent(boardName)}`, { cache: "no-store" });
      ...
      message.style.cssText = "position:fixed;left:16px;top:16px;color:#900;font:14px Arial,sans-serif;z-index:999999";
  ...
  function saveItem(item) {
    // Queue saves so quick interactions cannot arrive out of order.
    saveQueue = saveQueue.then(async () => {
      const response = await fetch(`/api/boards/${encodeURIComponent(boardName)}/items/${encodeURIComponent(item.id)}`, {
        method: "PUT",
  ...
  async function addItem(data) {
    ...
    const response = await fetch(`/api/boards/${encodeURIComponent(boardName)}/items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  function select(el) {
    document.querySelectorAll(".image-item.selected")
  ...
  async function uploadImage(file) {
    ...
    try {
      await addItem({ id, src: signed.signedUrl, storagePath, width, height, x: offset, y: offset });
    } catch (error) {
