...
- Segl activation applies across ordinary browser tabs during the gathering excursion. The user should not need to re-arm Segl for each tab.
...
- Clicking the sickle gathers that one thing silently into poki. Gathering must not automatically open poki, shift the webpage, or interrupt browsing.
- The browser-extension control may expose the current poki count unobtrusively while the bag remains closed.
- Opening the Eskja extension reveals poki on demand as a compact temporary control surface. From there the user may inspect the current carrying set, remove accidental gathers, clear it, toggle Segl, or press `keep`.
...
**2026-08-19** - Name the browser gathering implement **Segl** and the temporary carrying bag **poki**. The core interaction is physical gathering rather than crawling: the field is already there; the user notices a thing, reaches for Segl, gathers it, carries it in poki, and brings it home. Hover must be nearly free. Clicking Segl gathers only the chosen thing. Poki may carry additional gathered things and quick notes. `keep` sends the current poki home, by default into a new eskja so things gathered together arrive together. Segl must not scan, prefetch, probe, or enumerate the rest of a page merely because it can. Prefer browser-known resources; defer expensive resolution/source recovery until necessary or explicitly requested. Research bookmarklets that crawl profiles/pages are prototypes and utilities, not the default Segl product behavior.

**2026-08-19 (later)** - The browser-extension interaction should behave as one Eskja tool rather than separate extension/sidebar concepts. Segl activation persists globally across ordinary browser tabs until turned off. Gathering into poki is silent and must not open a panel or shift the page. The extension's toolbar control may show the current poki count and opens poki only when the user asks for it, as a compact temporary popup containing Segl on/off, the carrying set, remove/clear, and `keep`.
